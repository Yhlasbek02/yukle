const { TransportationType } = require("../../models/models");

class TransportationTypeController {
    async add(req, res) {
        try {
            const { nameEn, nameRu, nameTr, nameTm } = req.body;
            const type = await TransportationType.create({ nameEn, nameRu, nameTr, nameTm });
    ...
    async edit(req, res) {
        try {
            const { id } = req.params;
            const { nameEn, nameRu, nameTr, nameTm } = req.body;
            const type = await TransportationType.findOne({ where: { uuid: id } });
            if (!type) {
    ...
    async getAllType(req, res) {
        try {
            ...
            const { count, rows: types } = await TransportationType.findAndCountAll(queryOptions);
...
module.exports = TransportationTypeController;